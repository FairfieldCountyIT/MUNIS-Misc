...
CREATE VIEW [custom].[bss_elig_output] AS SELECT 
    ...
    , rtrim(COALESCE(bss_locations.OverrideLocationCode, bss_locations.LocationCode, prempmst.prem_loc,'')) AS DeptNo
    , rtrim(COALESCE(bss_locations.LocationDescription, prlocatn.prln_long,'')) AS Dept
    , rtrim(prempmst.prem_p_bargain) AS BargainUnit
...
LEFT OUTER JOIN mu_live.custom.bss_locations bss_locations ON prempmst.prem_loc = bss_locations.LocationCode
...
AND (bss_locations.DoSkip IS NULL OR bss_locations.DoSkip <> 'Y')
